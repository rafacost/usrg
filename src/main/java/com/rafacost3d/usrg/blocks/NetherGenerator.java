...
public class NetherGenerator extends Block {
    ...
    public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> tooltip, ITooltipFlag flags) {
        ...
        if (InputMappings.isKeyDown(Minecraft.getInstance().getMainWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
            ...
            if (information != null) {
                ...
                text = text.replace("{item}", NetherGeneratorTile.GENERATION_BLOCK.getTranslatedName().getString());
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new NetherGeneratorTile();
    }
}
