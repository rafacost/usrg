...
public class GlowstoneGenerator extends Block {
    public GlowstoneGenerator(){
        super(Properties.create(Material.ROCK)
                .sound(SoundType.STONE)
                .hardnessAndResistance(2.0f)
        );
        setRegistryName("glowstonegenerator");
    }
        ...
        if (InputMappings.isKeyDown(Minecraft.getInstance().getMainWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
            ...
            if (information != null) {
                ...
                if (Config.GENERATE_DUST.get()) {
                    text = text.replace("{item}", GlowstoneGeneratorTile.GENERATION_ITEM.getName().getString());
                } else {
                    text = text.replace("{item}", GlowstoneGeneratorTile.GENERATION_BLOCK.getTranslatedName().getString());
                }
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new GlowstoneGeneratorTile();
    }
}
