...
public class SnowGenerator extends Block {
        ...
        if (InputMappings.isKeyDown(Minecraft.getInstance().getMainWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
            ...
            if (information != null) {
                ...
                if (Config.GENERATE_DUST.get()) {
                    text = text.replace("{item}", SnowGeneratorTile.GENERATION_ITEM.getName().getString());
                } else {
                    text = text.replace("{item}", SnowGeneratorTile.GENERATION_BLOCK.getTranslatedName().getString());
                }
