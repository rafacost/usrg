...
public class RedstoneGenerator extends BaseGenerator {
    public RedstoneGenerator(){
        super();
        setRegistryName("redstonegenerator");
    }
        ...
        if (InputMappings.isKeyDown(Minecraft.getInstance().getMainWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
            ...
            if (information != null) {
                ...
                if (Config.GENERATE_DUST.get()) {
                    text = text.replace("{item}", RedstoneGeneratorTile.GENERATION_ITEM.getName().getString());
                } else {
                    text = text.replace("{item}", RedstoneGeneratorTile.GENERATION_BLOCK.getTranslatedName().getString());
                }
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new RedstoneGeneratorTile();
    }
}
