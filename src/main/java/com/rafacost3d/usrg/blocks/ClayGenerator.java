        ...
        if (InputMappings.isKeyDown(Minecraft.getInstance().getMainWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
            ...
            if (information != null) {
                ...
                if (Config.GENERATE_DUST.get()) {
                    text = text.replace("{item}", ClayGeneratorTile.GENERATION_ITEM.getName().getString());
                } else {
                    text = text.replace("{item}", ClayGeneratorTile.GENERATION_BLOCK.getTranslatedName().getString());
                }
